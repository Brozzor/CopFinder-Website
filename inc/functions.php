...
function new_transac($price, $comments,$options){
    include 'db.php';
    $ajd = time();
    if ($options == 'dep') {
        $insertmoinstok = $pdo->prepare("UPDATE settings SET reponses = reponses - $price WHERE id = 5");
        $price = $price * -1;
    }else{
        $insertmoinstok = $pdo->prepare("UPDATE settings SET reponses = reponses + $price WHERE id = 5");
    }
    $insertmoinstok->execute();
    $SQLlogs = $pdo -> prepare("INSERT INTO finance(price,comments,date_insert) VALUES('$price','$comments','$ajd')");
    $SQLlogs -> execute();
}

function settings_bsn($id){
    include 'db.php';
    $req = $pdo->prepare("SELECT * FROM settings WHERE id = ?");
    $req->execute(array($id));
    $settings = $req->fetch();
    return $settings->reponses;
}
...
function moyenne_money_reste(){
    return settings_bsn(5) / diff_day_timestamp(time(), strtotime("+1 month",first_date_month()));
}